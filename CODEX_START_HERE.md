...
- Các vai trò dự kiến: `learner`, `editor`, `reviewer`, `admin`.
- Các bảng đã thiết kế: users, courses, modules, lessons, vocabulary, lesson_vocabulary, lesson_progress, review_items, vip_plans, subscriptions, payment_orders, payment_events, content_versions và audit_logs.
...
1. Tách quyền và màn hình làm việc cho `editor`/`reviewer`; quy định rõ ai được chuyển `draft → review → published → archived`.
2. Chốt/mở rộng nội dung thật cho các lộ trình tiếp theo theo định dạng đã dùng cho “Văn phòng & hành chính”.
3. Bổ sung preview/khôi phục phiên bản bài học và bộ lọc audit cho admin.
4. Bổ sung audio đã được kiểm duyệt và object storage.
5. Khi có domain, cấu hình Resend, smoke-test email và lên lịch `npm run auth:cleanup`.
6. Hoàn thiện chủ thể kinh doanh, chính sách và quy trình hỗ trợ.
7. Tích hợp SePay sandbox/test, webhook idempotent và đối soát.
8. Kiểm thử bảo mật, backup, theo dõi lỗi rồi mới mở thanh toán thật.
9. Đo tỷ lệ hoàn thành bài đầu, quay lại ngày 7 và chuyển đổi VIP trước khi làm Flutter.
